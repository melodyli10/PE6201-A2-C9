...
def get_preauthorisation(
    ...
    if not candidates:
        return {
            ...
            "valid": False,
            "reason": (
                f"no pre-authorisation on file for member {member_id!r}, "
                f"procedure {procedure_code!r}"
            ),
        }
    ...
    # Found one or more, but none cover this date of service.
    pa = candidates[0]

    if date_of_service > pa["valid_to"]:
        reason = (
            f"{pa['preauth_id']} validity ended {pa['valid_to']}, "
            f"before date of service {date_of_service}"
        )
    else:
        reason = (
            f"{pa['preauth_id']} validity starts {pa['valid_from']}, "
            f"after date of service {date_of_service}"
        )
